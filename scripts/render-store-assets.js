  ...
  return new Promise((resolve, reject) => {
    ...
    server.listen(0, '127.0.0.1', () => {
      ...
      if (!address || typeof address === 'string') {
        reject(new Error('Store asset server did not receive a TCP port'));
        return;
      }
      resolve({
...
async function renderPopup(browser, baseUrl, name, variant) {
  ...
  await page.screenshot({
    path: path.join(popupDir, `${name}.png`),
    clip: { x: 0, y: 0, width: POPUP_W, height }
  });
  await context.close();
}
...
async function main() {
  ...
  try {
    for (const [name, variant] of Object.entries(POPUP_VARIANTS)) {
      await renderPopup(browser, server.baseUrl, name, variant);
    }
